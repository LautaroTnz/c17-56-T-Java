        ...
        <table className="table-auto border-collapse w-full rounded-xl">
          <thead>
            <tr className=" text-white xl:h-[40px] h-[51px] bg-celestediez sticky top-0 rounded-xl">
              <th className="px-4 py-2 text-[14px] text-texto xl:text-start xl:pl-11">
                Nombre
              </th>
              <th className="px-4 py-2 text-[14px] text-texto xl:text-start ">
                Especialidad
              </th>
              <th className="px-4 py-2 text-[14px] text-texto xl:text-center">
                Estado
              </th>
            </tr>
          </thead>
          <tbody>
            {data.map((row, index) => {
              ...
              const especialidad = especialidades.find(
                (especialidad) => especialidad.specialityId === row.speciality
              );
              return (
                <tr
                  key={index}
                  ...
                  <td className="text-[12px] flex justify-center items-center px-4 py-2 xl:mt-[15px] xl:mb-[15px]">
                    <span
                      className={`text-[12px] font-medium flex xl:px-4 xl:py-1.5 px-2.5 py-1 rounded-[30px] ${
                        row.active === true
                          ? "bg-activo text-activoletra"
                          : "bg-inactivo text-alertas"
                      }`}
                    >
