...
import { SearchbarRecepcionista, TableRegistrosMobile, TableRegistrosRecep } from "../../components";

function RegistroRecepcionista() {
  const dispatch = useDispatch();
  const { receptionists, loading, error } = useSelector((state) => state.receptionists);
      ...
      <div className='w-full mt-[80px] ml-4 mr-4 xl:mt-20 xl:ml-24 xl:mr-0'>
        ...
        <div>
          <div className='xl:hidden flex'>
            <TableRegistrosMobile dataRegistros={filteredReceptionists} /> {/* Ajustar los datos */}
          </div>
          <div className='hidden xl:flex w-[1152px] ml-[62px] mt-[45px]'>
            <TableRegistrosRecep dataRegistros={filteredReceptionists} /> {/* Ajustar los datos */}
          </div>
        </div>
      </div>
    </div>
